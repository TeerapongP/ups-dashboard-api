...
from app.api.routes.ups_config import UPS_DEVICES, DEFAULT_CONFIG
...
def read_all(timeout: float = Query(1.0, ge=0.2, le=10.0),
             ...
             ttl: float = Query(DEFAULT_CONFIG["cache_ttl"], ge=0.0, le=60.0)):
    items: List[Dict[str, Any]] = []
    with ThreadPoolExecutor(max_workers=workers) as ex:
        futs = [ex.submit(get_ups_data_cached, ip, cfg, timeout, retries, ttl)
                for ip, cfg in UPS_DEVICES.items()]
